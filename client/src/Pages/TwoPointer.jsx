...
function TwoPointer() {
    ...
    const binarySearch = async () => {
        ...
        while (low <= high) {
            const sum=parseInt(sortedArray[low])+parseInt(sortedArray[high]);

            setHighlightedIndices([low,high]);

            await new Promise(resolve => setTimeout(resolve, 500));

            if (sum=== parseInt(target)) {
                setMessage(`Target pair found ${sortedArray[low]},${sortedArray[high]}`);
                Setgreen([low,high]);
                setHighlightedIndices([low,high]);
                setIsRunning(false);
                return;
            } else if (sum< parseInt(target)) {
             low++;
            } else {
               high--;
            }
